...
async def create_private_group(eid):
    ...
    try:
        ...
        # Add the bot account/user to the created group.
        if BOT_USERNAME:
            try:
                ...
                print("bot invite:", exc)

        # Initial screenshot photo.
        if GROUP_INITIAL_PHOTO:
            try:
                ...
                print("initial photo:", exc)

        return link

    except Exception as exc:
        print("create_private_group:", exc)
        return None
...
async def type_callback(c: CallbackQuery):
    if c.data == "type_product":
        # Kept intentionally simple because the supplied screenshots
        # only define the P2P flow.
    ...
    link = await create_private_group(eid)
