...
async def create_escrow_group(e):
    ...
    try:
        # IMPORTANT:
        # This group and the messages inside it are owned/sent by the
        # Telegram USER account represented by STRING_SESSION.
        result = await client(
            CreateChannelRequest(
...
async def start(message: Message):
    ...
    # This is intentional: it is the separate selector shown below
    # the welcome block.
    await message.answer(
...
async def dd(message: Message):
    ...
    if e and e["group_id"]:
        try:
            ...
            print("DD rename warning:", exc)

    # Message shown inside the group is sent by the Telegram bot
    # account when /dd is handled through Bot API.
    await message.answer(
        "Hello there,\n"
